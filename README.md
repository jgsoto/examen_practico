...
El objetivo de esta rama es demostrar cómo la falta de estructura, aunque produce un software que "funciona", destruye la mantenibilidad y escalabilidad y del sistema.
...
* Las clases internas (`Book`, `Member`, `Loan`) actúan como contenedores de datos (estructuras anémicas).
* Sus atributos no son privados y no tienen métodos para controlar su estado. Cualquier parte del código puede modificar directamente propiedades críticas como `book.available = true` o `loan.returned = false` sin ninguna restricción ni validación previa.
...
* **Persistencia de Datos:** Gestión del almacenamiento en memoria a través de listas `ArrayList` locales.

### 3. Alto Acoplamiento y Baja Cohesión
* Todo el flujo del sistema depende de un ciclo infinito `while(true)` y un bloque `switch-case`.
...
``` 
2. Compilar el archivo principal desde la terminal o ejecutarlo directamente desde IntelliJ IDEA:
...
3. Seguir las instrucciones del menú en pantalla para registrar libros, miembros y gestionar préstamos.